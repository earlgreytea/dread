...
public sealed class OutlineRenderPass : ScriptableRenderPass
{
    ...
    private static readonly int BlurStandardDeviationId = Shader.PropertyToID(
    ...
    private static readonly int RenderObjectsTargetId = Shader.PropertyToID(
        "_OutlineRenderedObjectsMaskTexture"
    );
    private static readonly int BlurredRenderObjectsTargetId = Shader.PropertyToID(
        "_OutlineBlurredRenderedObjectsMaskTexture"
    );
    ...
    public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
    {
        ...
        CreateRenderGraphTextures(
            renderGraph,
            cameraData,
            out TextureHandle outlineCombinedMask,
            out TextureHandle horizontalBlurTarget,
            out TextureHandle verticalBlurTarget,
            out TextureHandle resolveTarget
        );

        using (
            IUnsafeRenderGraphBuilder builder = renderGraph.AddUnsafePass(
                "Outline Render Objects Pass",
        ...
        )
        {
            ...
            passData.rendererListHandleR = CreateRendererList(
                renderGraph,
                renderingData,
                cameraData,
                RenderingLayerMaskR,
            ...
            passData.rendererListHandleG = CreateRendererList(
                renderGraph,
                renderingData,
                cameraData,
                RenderingLayerMaskG,
            ...
            passData.rendererListHandleB = CreateRendererList(
                renderGraph,
                renderingData,
                cameraData,
                RenderingLayerMaskB,
            ...
            passData.rendererListHandleA = CreateRendererList(
                renderGraph,
                renderingData,
                cameraData,
                RenderingLayerMaskA,
            ...
            builder.SetRenderFunc(
                (PassData data, UnsafeGraphContext context) =>
                    ExecuteUnsafeRenderOutlinePass(data, context)
            );
        }

        using (
            IRasterRenderGraphBuilder builder = renderGraph.AddRasterRenderPass(
                "Outline Horizontal Blur Pass",
                out PassData passData,
                profilingSampler
            )
        )
    ...
    private void CreateRenderGraphTextures(
        ...
        out TextureHandle nonBlurredCombinedMask,
        out TextureHandle horizontalBlurTarget,
        out TextureHandle verticalBlurTarget,
        out TextureHandle resolveTarget
    )
    {
        ...
        resolveTarget = UniversalRenderer.CreateRenderGraphTexture(
            renderGraph,
        ...
        nonBlurredCombinedMask = UniversalRenderer.CreateRenderGraphTexture(
            renderGraph,
            cameraTargetDescriptor,
            "_OutlineCombinedMask",
            false,
            FilterMode.Point
        );
        horizontalBlurTarget = UniversalRenderer.CreateRenderGraphTexture(
            renderGraph,
            cameraTargetDescriptor,
            "_OutlineHorizontalBlur",
            false,
            FilterMode.Bilinear
        );
        verticalBlurTarget = UniversalRenderer.CreateRenderGraphTexture(
            renderGraph,
            cameraTargetDescriptor,
            "_OutlineVerticalBlur",
            false,
            FilterMode.Bilinear
        );
    }
    ...
    private static void UpdateOutlineMaterialProperties(PassData passData)
    {
        ...
        {
            OutlineVolumeComponent outlineVolume =
            ...
            outlineMaterial.SetFloat(
                BlurStandardDeviationId,
                Mathf.Floor((float)outlineVolume.blurRadius * 0.5f)
            );
        }
        else if (stage == PassStage.Resolve)
        {
            ...
            Vector4 fallOffs = new Vector4(
                outlineVolume.fallOff1.value,
                outlineVolume.fallOff2.value,
                outlineVolume.fallOff3.value,
                outlineVolume.fallOff4.value
            );
            outlineMaterial.SetVector(OutlineFallOffsId, fallOffs);
            Vector4 fillAlphas = new Vector4(
                outlineVolume.fillAlpha1.value,
                outlineVolume.fillAlpha2.value,
                outlineVolume.fillAlpha3.value,
                outlineVolume.fillAlpha4.value
            );
            ...
            outlineMaterial.SetTexture(
                BlurredRenderObjectsTargetId,
                passData.blurredRenderObjectsTarget
            );
        }
    }
    ...
    /// <param name="passData"></param>
    /// <param name="context"></param>
    private static void ExecuteUnsafeRenderOutlinePass(
        PassData passData,
